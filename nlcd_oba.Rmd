...
# or_boundary <- map_state %>%
#     filter(region %in% "oregon")

# Ensure both OBA data and Oregon boundary share the same CRS
or_boundary <- map_state %>%
  filter(region %in% "oregon") %>%
  st_as_sf(coords = c("long", "lat"), crs = 4326) %>%
  st_transform(crs = st_crs(oba_sf))  # Transform Oregon boundary to same CRS as OBA
...
oba <- dplyr::select(oba, Genus, Species, Dec..Lat., Dec..Long., Elevation..m.)
...
str(or_boundary)
or_boundary_sf <- st_as_sf(or_boundary, coords = c("lon", "lat"), crs = crs(oba_sf))
...
oba_cropped_sf <- st_crop(oba_sf, or_boundary)

# Check the result
summary(oba_cropped_sf)

# Check the result
# summary(oba_cropped)
```
```{r}
# ISSUE: or boundary is made up of points, not line 
...
ggplot() +
  ...
  geom_sf(data = oba_cropped_sf, aes(color = "black"), size = 1) +
...
oba_cropped_sf <- st_transform(oba_cropped_sf, crs = 32610)
# crs(oba_cropped_sf)


# Generate buffers for each buffer size (in meters)
buffers_100m <- st_buffer(oba_cropped_sf, dist = 100)
buffers_500m <- st_buffer(oba_cropped_sf, dist = 500)
buffers_1000m <- st_buffer(oba_cropped_sf, dist = 1000)
...
```{r}
# Ensure both the raster and buffers share the same CRS
...
# Define a function to calculate Shannon diversity and create the diversity matrix
calculate_diversity <- function(buffers, raster) {
  ...
  # Add diversity metrics to buffers
  buffers <- buffers %>%
...
# As example: Plot 100m buffers colored by their Shannon diversity index
ggplot() +
  geom_sf(data = or_boundary, fill = NA, color = "black") +  # OR boundary
  geom_sf(data = buffers_100m, aes(fill = shannon_div), color = NA) +
...
elevation_bands <- cut(oba$Elev, breaks = c(0, 500, 1000, 1500, Inf), 
