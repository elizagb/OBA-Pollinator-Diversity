...
#created the 500m buffer
buffers_500m <- st_buffer(oba_sf, dist = 500)

#ensure that oba_sf and buffer has the same CRS

#add buffer_id column to buffer
buffers_500m$buffer_id <- seq_len(nrow(buffers_500m))

# buffers_500m_selected <- buffers_500m %>%
#   select(buffer_id)  # Here you select the buffer_id column or others as needed

# Now, add the geometry back
# buffers_500m_final <- st_sf(buffers_500m_selected, geometry = buffers_500m$geometry)
...
colnames(buffers_500m)

# Ensure `richness` is a named vector with buffer IDs
# Convert richness to tibble
richness_df <- tibble(
  buffer_id = as.integer(names(richness)),  # Convert names to integers (buffer IDs)
  richness = richness                        # Keep the richness values (integer type)
)
# colnames(richness_df)

# Join richness data back to the buffers
buffers_500m_join <- buffers_500m %>%
  left_join(richness_df, by = "buffer_id")


# Plot buffers with richness levels
ggplot() +
  geom_sf(data = buffers_500m, aes(fill = richness), color = "black", size = 0.1) +
