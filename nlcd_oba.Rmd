...
bee_diversity <- diversity(community_matrix, index = "shannon")
...
land_cover_in_buffers <- extract(nlcd_resampled, buffers_500m, fun = function(x) {
  ...
  if (length(x) > 0) {
    ...
    shannon_index <- diversity(as.numeric(land_cover_count), index = "shannon")
...
ggplot() +
  ...
  # Plot the 500m buffers around OBA data
  geom_sf(data = buffers_500m, fill = "red", alpha = 0.3, color = "red") +
